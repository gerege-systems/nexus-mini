...
1. **Эхний ажиллуулалт бэлэн, дэлхийн стандартаар CLI-ээр** — татаж аваад
   `nexus-mini setup` (интерактив: DB холболт → role/сан → миграц → админ
   бүртгэл) хийгээд `nexus-mini serve`-ээр асаана. Вэб талд setup wizard
   БАЙХГҮЙ, landing хуудас төлөв шалгадаггүй (эхний хувилбарын
   `GET /api/setup`-ийг хуудас бүр дуудаж байсныг хэрэглэгч буруушааж
   CLI рүү шилжүүлсэн). Docker compose нь `ADMIN_*` хувьсагчаар админаа
   bootstrap хийнэ.
...
| Үе | Агуулга |
|---|---|
| 1 | Цөм: миграц + setup wizard + session auth + tenant + RBAC + audit + module SDK + devices модуль + app store (локал каталог) + landing/portal/админ панель |
...
| Хэсэг | Хаяг | Порт |
|---|---|---|
| Web (landing+portal+admin) | nexus-mini.runestonetechnologies.com | 3020 |
| API | nexus-mini-api.runestonetechnologies.com | 8084 |
