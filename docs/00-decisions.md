...
1. **Эхний ажиллуулалт: бүх тохиргоо env файлд** — DB холболтууд +
   ...
   **2026-09-06 (хэрэглэгчийн хүсэлтээр)**: open-gerege-nexus-ийн шидтэнийг
   хуулав — env зам ХЭВЭЭР, дээр нь `/setup` вэб зам нэмэгдэв: платформ
   админгүй үед API асахад 256-бит токен санах ойд үүсгэж зөвхөн логт бичнэ
   (`PORTAL_URL/setup?token=…`), буруу токенд 404, админ үүсмэгц токен устана;
   DB тал (`auth_setup_admin`) хоёр дахь админыг өөрөө татгалзана. Landing/login
   `/api/setup/status`-г асууж шаардлагатай бол `/setup` руу шилжүүлнэ.
