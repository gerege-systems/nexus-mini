...
**Хамрах хүрээ (cross-package): 63.4%.** Тестгүй ЗОРИУД үлдсэн: `cmd/nexus-mini`
(нэг мөр main), `core.Run`/`cmdServe`/`cmdMigrate` (амьд сервер, процессын
оролт — deploy дараах curl/Playwright, `make migrate`-аар шалгагддаг),
`cmd/nexus`-ийн сүлжээ/Go toolchain шаардах хэсэг (`init`/`add`/`upgrade` —
бодит дистрибуц дээр гараар батлагдсан), `db.Connect/Close` (pool амьдрал).
