...
- Сервер дээр: `bash deploy/deploy.sh` — pull (deploy.sh өөрөө өөрчлөгдвөл шинэ хувилбараар өөрийгөө exec) → `pg_dump -Fc` (`NEXUS_BACKUP_DIR`, default `/var/backups/nexus-mini`, сүүлийн 14 үлдэнэ) → go build (атом mv, `.prev` үлдээнэ) → `migrate --env <secrets>/nexus-mini.env` → frontend/admin `pnpm build` (`.next.new` → атом солилт) → restart 3 unit → health curl.
